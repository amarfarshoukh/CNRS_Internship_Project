...
# =============================
# LOAD LEBANON LOCATIONS FROM DB
# =============================
DB_PATH = r"C:\Users\user\OneDrive - Lebanese University\Desktop\lebanon_locations.db"

# Create DB from CSV if not exists
if not os.path.exists(DB_PATH):
    df = pd.read_csv(r"C:\Users\user\OneDrive - Lebanese University\Desktop\lebanon_locations-arabic.csv", encoding='utf-8')
    conn = sqlite3.connect(DB_PATH)
    df.to_sql("locations", conn, if_exists="replace", index=False)
    conn.close()

# Load locations into dictionary
conn = sqlite3.connect(DB_PATH)
cursor = conn.cursor()
LEBANON_LOCATIONS = {}
...
async def main():
    ...
    # Load or initialize matched messages
